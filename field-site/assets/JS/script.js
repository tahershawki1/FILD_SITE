    ...
    */
    ...
    function renderHomeCards(){
      $("#cards").innerHTML = TASKS.map(t => `
        <a class="cardLink" href="#" data-open="${escapeHtml(t.id)}">
          <h3 class="cardTitle">${escapeHtml(t.title)}</h3>
          <p class="cardDesc">${escapeHtml(t.desc)}</p>
        </a>
      `).join("");
    }
    ...
    function newLevelStep3HTML(d){
      ...
      return `
        <section class="card">
          ...
          <h2 class="h2">4) Gate Level Reading</h2>
          ...
          <div class="resultBox" style="margin-top:12px; direction:ltr">
            <b>Final Report</b><br>
            Offset = ${plusText}<br>
